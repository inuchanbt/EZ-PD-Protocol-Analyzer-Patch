...
/** Re-registers USB PD table renderers after every application configuration step. */
public final class FinalTableConfigurationPatcher {
    ...
    private FinalTableConfigurationPatcher() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            throw new IllegalArgumentException(
                "usage: FinalTableConfigurationPatcher <input.class> <output.class>"
            );
        }
                ...
                return new MethodVisitor(Opcodes.ASM8, downstream) {
                    ...
                    public void visitInsn(int opcode) {
                        if (opcode == Opcodes.ARETURN) {
                            // This is deliberately the last operation in table
                            // construction. Row-header configuration can occur
                            // later than the theme setup patched in v1.0h.
                            super.visitVarInsn(Opcodes.ALOAD, 0);
                            super.visitFieldInsn(
                                Opcodes.GETFIELD, TABLE, "natTable",
                                "L" + NAT_TABLE + ";"
                            );
        ...
        if (methods[0] != 1 || inserts[0] != 1) {
            throw new IllegalStateException(
                "Unexpected final configuration patch points: methods=" + methods[0] +
                ", inserts=" + inserts[0]
            );
        }
        Files.write(Path.of(args[1]), writer.toByteArray());
        System.out.println("Patched final USB PD table renderer registration.");
    }
}
